...
interface ComposerDraftStoreState {
  ...
  getDraftThreadByProjectId: (projectId: ProjectId | null) => ProjectDraftThread | null;
  ...
  addFiles: (threadId: ThreadId, files: ChatFileAttachment[]) => void;
...
const EMPTY_FILES: ChatFileAttachment[] = [];
...
export function deriveEffectiveComposerModelState(input: {
  ...
  const persistedModel = input.threadModelSelection?.model ?? input.projectModelSelection?.model;
  const baseModel =
...
function normalizePersistedFileAttachment(value: unknown): ChatFileAttachment | null {
  if (!value || typeof value !== "object") {
    return null;
  }
  const candidate = value as Record<string, unknown>;
  if (
    ...
    candidate.storageName.length === 0
  ) {
    return null;
  }
  return {
...
function partializeComposerDraftStoreState(
  ...
  for (const [threadId, draft] of Object.entries(state.draftsByThreadId)) {
    ...
    const persistedDraft: DeepMutable<PersistedComposerThreadDraftState> = {
      ...
      ...(draft.files.length > 0 ? { fileAttachments: draft.files } : {}),
      ...(draft.terminalContexts.length > 0
        ? {
...
function toHydratedThreadDraft(
  ...
  return {
    ...
    files: (persistedDraft.fileAttachments ?? []).flatMap((attachment) => {
      const normalized = normalizePersistedFileAttachment(attachment);
      return normalized ? [normalized] : [];
    }),
    nonPersistedImageIds: [],
    ...
    (set, get) => ({
      ...
      setProjectDraftThreadId: (projectId, threadId, options) => {
        ...
        set((state) => {
          ...
          const previousThreadIdForProject =
            projectId === null ? undefined : state.projectDraftThreadIdByProjectId[projectId];
          ...
          if (
            ...
            nextProjectDraftThreadIdByProjectId[existingThread.projectId] === threadId
          ) {
            delete nextProjectDraftThreadIdByProjectId[existingThread.projectId];
          }
      ...
      setDraftThreadContext: (threadId, options) => {
        ...
        set((state) => {
          ...
          if (existing.projectId !== nextProjectId && existing.projectId !== null) {
      ...
      clearProjectDraftThreadById: (projectId, threadId) => {
        if (!projectId || projectId.length === 0 || threadId.length === 0) {
          return;
        }
        set((state) => {
      ...
      removeFile: (threadId, fileId) => {
        if (threadId.length === 0 || fileId.length === 0) {
          return;
        }
        set((state) => {
          const current = state.draftsByThreadId[threadId];
