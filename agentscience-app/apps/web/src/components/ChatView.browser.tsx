...
describe("ChatView timeline estimator parity (full app)", () => {
    ...
    try {
      await vi.waitFor(
        () => {
          ...
          expect(openRequest?.env?.AGENTSCIENCE_WORKTREE_PATH).toBeUndefined();
        },
        { timeout: 8_000, interval: 16 },
      );
    } finally {
  ...
  it("falls back to the first installed editor when the stored favorite is unavailable", async () => {
    localStorage.setItem("agentscience:last-editor", JSON.stringify("vscodium"));
    setDraftThreadWithoutWorktree();
      ...
      await vi.waitFor(
        () => {
          ...
          expect(openRequest).toMatchObject({
            ...
            env: {
              AGENTSCIENCE_PROJECT_ROOT: "/repo/project",
              AGENTSCIENCE_WORKTREE_PATH: "/repo/worktrees/feature-draft",
            },
          });
        },
        { timeout: 8_000, interval: 16 },
        ...
        if (body._tag === WS_METHODS.gitResolvePullRequest) {
          return {
            pullRequest: {
              ...
              url: "https://github.com/pingdotgg/agentscience/pull/1359",
              baseBranch: "main",
          ...
          expect(dispatchRequest).toMatchObject({
            ...
            bootstrap: {
              ...
              prepareWorktree: {
                ...
                baseBranch: "main",
                branch: expect.stringMatching(/^agentscience\/[0-9a-f]{8}$/),
              },
              runSetupScript: true,
            },
          });
          ...
          expect(
            wsRequests.some(
              (request) =>
                ...
                (request as { type?: string }).type === "thread.session.stop",
            ),
          ).toBe(true);
        },
        { timeout: 8_000, interval: 16 },
  ...
  it("shows the confirm archive action after clicking the archive button", async () => {
    localStorage.setItem(
      "agentscience:client-settings:v1",
      JSON.stringify({
        ...DEFAULT_CLIENT_SETTINGS,
    ...
    try {
      ...
      localStorage.removeItem("agentscience:client-settings:v1");
      await mounted.cleanup();
    }
  });
