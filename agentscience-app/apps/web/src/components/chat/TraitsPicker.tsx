...
import { trimOrNull, getDefaultEffort, resolveEffort } from "@agentscience/shared/model";
...
function getSelectedTraits(
  ...
  const contextWindowOptions: ReadonlyArray<{ value: string; label: string; isDefault?: boolean }> =
    [];
  const contextWindow = null;
  const defaultContextWindow = null;
  ...
  const ultrathinkInBodyText = false;
  ...
  const handleEffortChange = useCallback(
    (value: string) => {
      ...
      if (caps.promptInjectedEffortLevels.includes(nextOption.value)) {
        const nextPrompt = prompt.trim().length === 0 ? ULTRATHINK_PROMPT_PREFIX : prompt;
      ...
      updateModelOptions(
        buildNextOptions(provider, modelOptions, { reasoningEffort: nextOption.value }),
      );
    },
    [
      ultrathinkPromptControlled,
