...
function ThreadStatusDot({ status }: { status: ThreadStatusPill | null }) {
  ...
  return <StatusDot dotClass={status.dotClass} pulse={status.pulse} label={status.label} />;
}
...
export default function Sidebar() {
  ...
  const paperWorkflowModeByThreadId = useUiStateStore((state) => state.paperWorkflowModeByThreadId);
  ...
  const handleMovePaper = async (threadId: ThreadId, projectId: ProjectId | null) => {
    try {
  ...
  const handleThreadDragStart = (event: DragEvent<HTMLDivElement>, thread: SidebarThreadEntry) => {
    if (thread.isDraft) {
  ...
  const handleProjectDragOver = (event: DragEvent<HTMLDivElement>, projectId: ProjectId) => {
    if (!draggedThreadId) {
      return;
    }
  ...
  const handleProjectDrop = async (event: DragEvent<HTMLDivElement>, projectId: ProjectId) => {
    event.preventDefault();
  ...
  const contextThread = contextMenu ? visibleThreadsById.get(contextMenu.threadId) : undefined;
    ...
    <>
      <SidebarHeader className="flex shrink-0 flex-col gap-0 p-0">
        ...
        */}
        ...
        >
          <BrandMark size={28} wordmarkClassName="text-lg group-data-[collapsible=icon]:hidden" />
          <SidebarTrigger
                                ...
                                >
                                  ...
                                  <span className="truncate tabular-nums">
                                    ...
                                    {threadStatus?.startedAt
                                      ? (formatWorkingDuration(threadStatus.startedAt, nowIso) ??
                                        "0s")
              ...
              return (
                <div
                  ...
                  className={cn(
                    ...
                    dropProjectId === project.id && "rounded-md bg-sidebar-accent/40",
                  )}
                  onDragLeave={(event) => {
      ...
      <SidebarFooter className="border-t border-sidebar-border px-2 py-1 group-data-[collapsible=icon]:items-center">
        <SidebarMenu>
          <SidebarMenuItem>
            <SidebarMenuButton
              size="sm"
              isActive={location.pathname === "/papers" || location.pathname.startsWith("/papers/")}
      ...
      >
        <DialogPopup>
          <DialogHeader>
            ...
            <DialogDescription>Create a project enclosure for related papers.</DialogDescription>
          </DialogHeader>
