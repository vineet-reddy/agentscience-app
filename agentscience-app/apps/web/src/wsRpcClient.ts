...
import { applyGitStatusStreamEvent } from "@agentscience/shared/git";
...
export interface WsRpcClient {
  ...
  readonly attachments: {
    readonly importFiles: RpcUnaryMethod<typeof WS_METHODS.attachmentsImportFiles>;
  };
  readonly shell: {
  ...
  readonly server: {
    ...
    readonly logoutGemini: RpcUnaryNoArgMethod<typeof WS_METHODS.serverLogoutGemini>;
...
export function createWsRpcClient(transport = new WsTransport()): WsRpcClient {
  return {
    ...
    attachments: {
      importFiles: (input) =>
        transport.request((client) => client[WS_METHODS.attachmentsImportFiles](input)),
    },
    shell: {
      openInEditor: (input) =>
    ...
    server: {
      ...
      logoutGemini: () =>
        transport.request((client) => client[WS_METHODS.serverLogoutGemini]({})),
