...
} from "@agentscience/shared/git";

import { GitManagerError } from "@agentscience/contracts";
import {
  GitManager,
...
import type { GitManagerServiceError } from "@agentscience/contracts";
...
function resolvePullRequestWorktreeLocalBranchName(
  ...
  return `agentscience/pr-${pullRequest.number}/${suffix}`;
}
...
export const makeGitManager = Effect.fn("makeGitManager")(function* () {
  ...
  const runPrStep = Effect.fn("runPrStep")(function* (
    ...
    const bodyFile = path.join(tempDir, `agentscience-pr-body-${process.pid}-${randomUUID()}.md`);
    yield* fileSystem
