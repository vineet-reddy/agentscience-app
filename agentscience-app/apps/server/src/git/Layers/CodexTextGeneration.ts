...
import { CodexModelSelection } from "@agentscience/contracts";
import { sanitizeBranchFragment, sanitizeFeatureBranchName } from "@agentscience/shared/git";

import { resolveAttachmentPath } from "../../attachmentStore.ts";
import { ServerConfig } from "../../config.ts";
import { TextGenerationError } from "@agentscience/contracts";
import {
...
import { buildCodexSpawnEnv } from "../../provider/codexCli.ts";
import { resolveEffectiveCodexSettings } from "../../provider/codexSettings.ts";
...
const makeCodexTextGeneration = Effect.gen(function* () {
  ...
  const writeTempFile = (
    ...
    return fileSystem
      .makeTempFileScoped({
        prefix: `agentscience-${prefix}-${process.pid}-${randomUUID()}.tmp`,
      })
      .pipe(
  ...
  const runCodexJson = Effect.fn("runCodexJson")(function* <S extends Schema.Top>({
    ...
    const codexSettings = yield* Effect.map(
      ...
      (settings) => resolveEffectiveCodexSettings(settings.providers.codex, serverConfig),
