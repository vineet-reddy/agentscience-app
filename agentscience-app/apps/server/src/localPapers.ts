...
type PublishBundlePolicy = {
  readonly name: "standard" | "figures-only" | "paper-only";
...
const FIGURES_ONLY_PUBLISH_BUNDLE_POLICY: PublishBundlePolicy = {
  name: "figures-only",
  command: "publish --bundle figures-only",
  description: "Retry with the paper PDF, source, bibliography metadata, and figures only.",
...
const SMART_PUBLISH_POLICIES = [
  ...
  FIGURES_ONLY_PUBLISH_BUNDLE_POLICY,
...
export const makeLocalPapersService = Effect.gen(function* () {
  ...
  const startPublishRepairAgent = (input: {
    ...
    Effect.gen(function* () {
      ...
      if (!input.paper.threadId) {
        yield* orchestrationEngine.dispatch({
          type: "thread.create",
          commandId: CommandId.makeUnsafe(`cmd-smart-publish-thread-${randomUUID()}`),
          threadId,
      ...
      const startTurn = orchestrationEngine.dispatch({
        ...
        yield* startTurn.pipe(
          Effect.catch((error) =>
            orchestrationEngine
              .dispatch({
                type: "thread.delete",
                commandId: CommandId.makeUnsafe(`cmd-smart-publish-cleanup-${randomUUID()}`),
                threadId,
              })
              .pipe(
    ...
    Effect.gen(function* () {
      ...
      if (repairResult.ok) {
        ...
        return {
          ...
          paper,
          steps,
          repairThreadId: repairResult.threadId,
          error:
