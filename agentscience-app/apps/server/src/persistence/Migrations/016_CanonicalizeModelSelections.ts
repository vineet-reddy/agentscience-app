          ...
          json_patch(
            json_object(
              ...
              CASE
                ...
                WHEN json_type(payload_json, '$.defaultModelOptions') = 'object'
                  AND EXISTS(
                    SELECT 1
                    FROM json_each(payload_json, '$.defaultModelOptions')
                  )
                THEN json_object(
                  'options',
                  json(
                    (
                      SELECT value
                      FROM json_each(payload_json, '$.defaultModelOptions')
                      ORDER BY CASE WHEN key = 'codex' THEN 0 ELSE 1 END
                      LIMIT 1
                    )
                  )
                )
      ...
      json_set(
        ...
        json_patch(
          ...
          CASE
            ...
            WHEN json_type(payload_json, '$.modelOptions') = 'object'
              AND EXISTS(
                SELECT 1
                FROM json_each(payload_json, '$.modelOptions')
              )
            THEN json_object(
              'options',
              json(
