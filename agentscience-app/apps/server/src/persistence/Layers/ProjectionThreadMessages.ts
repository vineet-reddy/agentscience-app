...
import { ChatAttachment, SuggestedAction } from "@agentscience/contracts";
...
function toProjectionThreadMessage(
  ...
  return {
    ...
    ...(row.suggestedActions !== null ? { suggestedActions: row.suggestedActions } : {}),
  };
}
...
const makeProjectionThreadMessageRepository = Effect.gen(function* () {
  ...
  const upsertProjectionThreadMessageRow = SqlSchema.void({
    ...
    execute: (row) => {
      ...
      const nextSuggestedActionsJson =
        row.suggestedActions !== undefined ? JSON.stringify(row.suggestedActions) : null;
      return sql`
        ...
        VALUES (
          ...
          COALESCE(
            ${nextSuggestedActionsJson},
            (
              SELECT suggested_actions_json
              FROM projection_thread_messages
              WHERE message_id = ${row.messageId}
            )
          ),
        ...
        DO UPDATE SET
          ...
          suggested_actions_json = COALESCE(
            excluded.suggested_actions_json,
            projection_thread_messages.suggested_actions_json
          ),
