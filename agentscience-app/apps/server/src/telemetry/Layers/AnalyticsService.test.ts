  ...
  it.effect("flush drains all buffered events across multiple batches", () =>
    Effect.gen(function* () {
      ...
      const serverConfigLayer = ServerConfig.layerTest(process.cwd(), {
        prefix: "agentscience-telemetry-base-",
      });

      const telemetryLayer = AnalyticsServiceLayerLive.pipe(Layer.provideMerge(serverConfigLayer));
      const configLayer = ConfigProvider.layer(
        ConfigProvider.fromUnknown({
          AGENTSCIENCE_TELEMETRY_ENABLED: true,
          AGENTSCIENCE_POSTHOG_KEY: "phc_test_key",
          AGENTSCIENCE_POSTHOG_HOST: "",
          AGENTSCIENCE_TELEMETRY_FLUSH_BATCH_SIZE: 20,
        }),
      );
