...
describe("ProjectionSnapshotQuery", () => {
  ...
  it("preserves per-thread ordering and caps for interleaved snapshot rows", async () => {
    const MAX_MESSAGES = 2_000;
    const MAX_PROPOSED_PLANS = 200;
    const threadIds = ["thread-alpha", "thread-beta"] as const;
    ...
    const snapshot = await Effect.gen(function* () {
      ...
      yield* sql`
        INSERT INTO projection_projects (
          ...
          'project-interleaved', 'Interleaved Project', 'interleaved-project', NULL, '[]',
          '2026-03-02T00:00:00.000Z', '2026-03-02T00:00:01.000Z', NULL
        )
      `;

      for (const [threadIndex, threadId] of threadIds.entries()) {
        yield* sql`
          INSERT INTO projection_threads (
            thread_id, project_id, folder_slug, title, branch, worktree_path, runtime_mode, interaction_mode,
            model_selection_json, latest_turn_id, created_at, updated_at, archived_at, deleted_at
          ) VALUES (
            ${threadId}, 'project-interleaved', ${threadId}, ${`Thread ${threadIndex}`}, NULL, NULL,
            'full-access', 'default', '{"provider":"codex","model":"gpt-5.4"}', NULL,
            ${isoAt(threadIndex)}, ${isoAt(10 + threadIndex)}, NULL, NULL
          )
        `;
      }

      for (let index = 1; index <= MAX_MESSAGES + 3; index += 1) {
        for (const threadId of threadIds) {
          const createdAt = isoAt(1_000 + index);
          yield* sql`
            INSERT INTO projection_thread_messages (
              message_id, thread_id, turn_id, role, text, attachments_json, is_streaming, created_at, updated_at
            ) VALUES (
      ...
      for (let index = 1; index <= MAX_PROPOSED_PLANS + 3; index += 1) {
        for (const threadId of threadIds) {
          const createdAt = isoAt(10_000 + index);
          yield* sql`
            INSERT INTO projection_thread_proposed_plans (
      ...
      yield* sql`
        ...
        (
          'thread-alpha', 'thread-alpha-turn-incomplete', NULL, NULL, NULL, NULL,
          'completed', ${isoAt(20_000)}, ${isoAt(20_001)}, NULL,
          NULL, NULL, NULL, '[]'
        ),
        (
          'thread-alpha', 'thread-alpha-turn-2', NULL, NULL, NULL, 'thread-alpha-message-2',
          'completed', ${isoAt(20_002)}, ${isoAt(20_003)}, ${isoAt(20_004)},
          2, 'checkpoint-alpha-2', 'ready',
          '[{"path":"src/b.ts","kind":"modified","additions":2,"deletions":0}]'
        ),
        (
          'thread-alpha', 'thread-alpha-turn-1', NULL, NULL, NULL, 'thread-alpha-message-1',
          'completed', ${isoAt(20_005)}, ${isoAt(20_006)}, ${isoAt(20_007)},
          1, 'checkpoint-alpha-1', 'ready',
          '[{"path":"src/a.ts","kind":"modified","additions":1,"deletions":0}]'
        ),
        (
          'thread-beta', 'thread-beta-turn-1', NULL, NULL, NULL, 'thread-beta-message-1',
          'completed', ${isoAt(21_000)}, ${isoAt(21_001)}, ${isoAt(21_002)},
          1, 'checkpoint-beta-1', 'ready',
          '[{"path":"src/beta.ts","kind":"added","additions":1,"deletions":0}]'
        )
      `;

      yield* sql`
        INSERT INTO projection_state (projector, last_applied_sequence, updated_at) VALUES
        ('projection.projects', 1, '2026-03-02T00:00:01.000Z'),
        ('projection.threads', 1, '2026-03-02T00:00:01.000Z'),
        ('projection.thread-messages', 1, '2026-03-02T00:00:01.000Z')
      `;
    ...
    expect(snapshot.threads.map((thread) => thread.id)).toEqual([
      ThreadId.makeUnsafe("thread-alpha"),
      ThreadId.makeUnsafe("thread-beta"),
    ]);

    for (const threadId of threadIds) {
      const thread = snapshot.threads.find((entry) => entry.id === ThreadId.makeUnsafe(threadId));
      expect(thread).toBeDefined();
      if (!thread) continue;

      expect(thread.messages).toHaveLength(MAX_MESSAGES);
      expect(thread.messages[0]?.id).toEqual(MessageId.makeUnsafe(`${threadId}-message-4`));
      expect(thread.messages.at(-1)?.id).toEqual(
        MessageId.makeUnsafe(`${threadId}-message-2003`),
      );

      expect(thread.proposedPlans).toHaveLength(MAX_PROPOSED_PLANS);
      expect(thread.proposedPlans[0]?.id).toBe(`${threadId}-plan-4`);
      expect(thread.proposedPlans.at(-1)?.id).toBe(`${threadId}-plan-203`);
    }

    const alpha = snapshot.threads.find(
      (entry) => entry.id === ThreadId.makeUnsafe("thread-alpha"),
    );
    expect(alpha?.checkpoints.map((checkpoint) => checkpoint.turnId)).toEqual([
      TurnId.makeUnsafe("thread-alpha-turn-1"),
      TurnId.makeUnsafe("thread-alpha-turn-2"),
    ]);
  });
});
