  ...
  if (typeof error === "object" && error !== null) {
    ...
    return {
      ...
      ...(record.cause !== undefined
        ? { cause: describeErrorForLog(record.cause) }
        : {}),
    };
  }
...
function parseAgentScienceMessageMetadata(
  valueJson: string,
): AgentScienceMessageMetadata | null {
  try {
...
const makeOrchestrationProjectionPipeline = Effect.fn(
  ...
  const upsertProjectMetadata = Effect.fn("upsertProjectMetadata")(
    function* (input: {
      readonly projectId: string;
      readonly defaultModelSelection: OrchestrationEvent["type"] extends never
        ? never
        : unknown;
      readonly scripts: ReadonlyArray<unknown>;
      readonly deletedAt: string | null;
  ...
  const upsertThreadMetadata = Effect.fn("upsertThreadMetadata")(
    function* (input: {
  ...
  const upsertResearchProject = Effect.fn("upsertResearchProject")(
    function* (input: {
      readonly projectId: string;
      readonly folderSlug: string;
      readonly title: string;
  ...
  const upsertResearchMessage = Effect.fn("upsertResearchMessage")(
    function* (input: {
      readonly messageId: string;
      readonly threadId: string;
      ...
      const nextAttachments =
        input.attachments ?? previousMetadata?.attachments;
    ...
    Effect.gen(function* () {
      switch (event.type) {
        ...
        case "project.meta-updated": {
          ...
          const existingResearchProject = yield* getResearchProjectRow(
      ...
      Effect.mapError(
        toProjectionSqlOrPersistenceError(
          "ProjectionPipeline.applyProjectsProjection:query",
        ),
      ),
    );
  ...
  const applyThreadsProjection: ProjectorDefinition["apply"] = (
    ...
    Effect.gen(function* () {
          ...
          if (
            ...
            yield* upsertThreadMetadata({
              ...
              modelSelection:
                ...
                existingThreadMetadata.modelSelection,
              runtimeMode: existingThreadMetadata.runtimeMode,
              interactionMode: existingThreadMetadata.interactionMode,
              branch:
      ...
      Effect.mapError(
        toProjectionSqlOrPersistenceError(
          "ProjectionPipeline.applyThreadsProjection:query",
        ),
      ),
    );
