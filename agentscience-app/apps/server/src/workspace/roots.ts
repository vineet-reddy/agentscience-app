...
export function isContainerWorkspaceRoot(
...
export function validatePaperWorkspaceRoot(input: {
  ...
  if (input.projectWorkspaceRoot === undefined || input.projectWorkspaceRoot === null) {
    ...
    if (path.dirname(normalizedWorkspaceRoot) !== unmanagedPapersRoot) {
      return {
        ok: false,
        workspaceRoot: normalizedWorkspaceRoot,
        detail: "Unassigned paper workspace root must be a direct child of the managed Papers container.",
      };
    }
    return {
      ok: true,
  ...
  if (path.dirname(normalizedWorkspaceRoot) !== managedProjectPapersRoot) {
    return {
      ok: false,
      workspaceRoot: normalizedWorkspaceRoot,
      detail: "Project paper workspace root must be a direct child of the managed project papers container.",
    };
  }
  return {
    ok: true,
