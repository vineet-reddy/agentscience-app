...
const BootstrapEnvelopeSchema = Schema.Struct({
  ...
  agentScienceHome: Schema.optional(Schema.String),
...
const baseDirFlag = Flag.string("base-dir").pipe(
  Flag.withDescription("Base directory path (equivalent to AGENTSCIENCE_HOME)."),
  Flag.optional,
);
...
const logWebSocketEventsFlag = Flag.boolean("log-websocket-events").pipe(
  Flag.withDescription(
    "Emit server-side logs for outbound WebSocket push traffic (equivalent to AGENTSCIENCE_LOG_WS_EVENTS).",
  ),
...
const EnvServerConfig = Config.all({
  logLevel: Config.logLevel("AGENTSCIENCE_LOG_LEVEL").pipe(Config.withDefault("Info")),
  traceMinLevel: Config.logLevel("AGENTSCIENCE_TRACE_MIN_LEVEL").pipe(Config.withDefault("Info")),
  traceTimingEnabled: Config.boolean("AGENTSCIENCE_TRACE_TIMING_ENABLED").pipe(
    Config.withDefault(true),
  ),
  traceFile: Config.string("AGENTSCIENCE_TRACE_FILE").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  traceMaxBytes: Config.int("AGENTSCIENCE_TRACE_MAX_BYTES").pipe(
    Config.withDefault(10 * 1024 * 1024),
  ),
  traceMaxFiles: Config.int("AGENTSCIENCE_TRACE_MAX_FILES").pipe(Config.withDefault(10)),
  traceBatchWindowMs: Config.int("AGENTSCIENCE_TRACE_BATCH_WINDOW_MS").pipe(
    Config.withDefault(200),
  ),
  otlpTracesUrl: Config.string("AGENTSCIENCE_OTLP_TRACES_URL").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  otlpMetricsUrl: Config.string("AGENTSCIENCE_OTLP_METRICS_URL").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  otlpExportIntervalMs: Config.int("AGENTSCIENCE_OTLP_EXPORT_INTERVAL_MS").pipe(
  ...
  port: Config.port("AGENTSCIENCE_PORT").pipe(Config.option, Config.map(Option.getOrUndefined)),
  host: Config.string("AGENTSCIENCE_HOST").pipe(Config.option, Config.map(Option.getOrUndefined)),
  agentScienceHome: Config.string("AGENTSCIENCE_HOME").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  devUrl: Config.url("VITE_DEV_SERVER_URL").pipe(Config.option, Config.map(Option.getOrUndefined)),
  noBrowser: Config.boolean("AGENTSCIENCE_NO_BROWSER").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  authToken: Config.string("AGENTSCIENCE_AUTH_TOKEN").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  bootstrapFd: Config.int("AGENTSCIENCE_BOOTSTRAP_FD").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  autoBootstrapProjectFromCwd: Config.boolean("AGENTSCIENCE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  logWebSocketEvents: Config.boolean("AGENTSCIENCE_LOG_WS_EVENTS").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
});
  ...
  Effect.gen(function* () {
    ...
    const baseDir = yield* resolveBaseDir(
      Option.getOrUndefined(
        resolveOptionPrecedence(
          flags.baseDir,
          Option.fromUndefinedOr(env.agentScienceHome),
          Option.flatMap(bootstrapEnvelope, (bootstrap) =>
            Option.fromUndefinedOr(bootstrap.agentScienceHome),
          ),
        ),
      ),
    );
...
const rootCommand = Command.make("agentscience-server", commandFlags).pipe(
  Command.withDescription("Run the Agent Science server."),
