...
describe("paper review helpers", () => {
  ...
  it("finds manuscripts in nested research workspaces", async () => {
    ...
    await fs.writeFile(path.join(manuscriptWorkspaceRoot, "paper.tex"), "\\section{Intro}\n", "utf8");
  ...
  it("finds manuscripts in nested manuscript directories", async () => {
    ...
    await fs.writeFile(path.join(manuscriptWorkspaceRoot, "paper.tex"), "\\section{Intro}\n", "utf8");
    await fs.writeFile(path.join(manuscriptWorkspaceRoot, "paper.pdf"), "pdf", "utf8");
    await fs.writeFile(path.join(manuscriptWorkspaceRoot, "references.bib"), "@article{demo}\n", "utf8");
  ...
  it("prefers explicit manuscript presentation activities over directory heuristics", async () => {
    ...
    await fs.writeFile(path.join(deepManuscriptWorkspaceRoot, "paper.tex"), "\\section{Intro}\n", "utf8");
  ...
  it("starts background latex builds automatically and keeps the last pdf preview visible", async () => {
    vi.resetModules();

    let releaseBuild: (() => void) | null = null;
    let completeBuildTracking: (() => void) | null = null;
    let workspaceRoot: string | null = null;
    const buildCompleted = new Promise<void>((resolve) => {
      completeBuildTracking = resolve;
    });
    ...
    vi.doMock("node:child_process", async (importOriginal) => {
      ...
      return {
        ...
        spawnSync: vi.fn((command: string) => {
          ...
          return { error: new Error("missing") };
        }),
      };
    });

    vi.doMock("./processRunner", () => ({
      runProcess: vi.fn(() => {
        return new Promise((resolve) => {
          const finishBuildTracking = completeBuildTracking;
          releaseBuild = () => {
            resolve({
              stdout: "latexmk ok",
              stderr: "",
    ...
    try {
      ...
      const { OrchestrationEngineService } = await import(
        "./orchestration/Services/OrchestrationEngine"
      );

      workspaceRoot = await fs.mkdtemp(
        path.join(os.tmpdir(), "agentscience-paper-review-autobuild-"),
      );
      await fs.writeFile(path.join(workspaceRoot, "paper.pdf"), "pdf", "utf8");
      await fs.writeFile(path.join(workspaceRoot, "paper.tex"), "\\section{Intro}\n", "utf8");

      const pdfTime = new Date("2026-04-15T12:00:00.000Z");
      const texTime = new Date("2026-04-15T12:05:00.000Z");
      await fs.utimes(path.join(workspaceRoot, "paper.pdf"), pdfTime, pdfTime);
      await fs.utimes(path.join(workspaceRoot, "paper.tex"), texTime, texTime);
      ...
      const snapshot = await Effect.runPromise(program);
      expect(snapshot.compile.status).toBe("compiling");
      expect(snapshot.compile.needsBuild).toBe(true);
      expect(snapshot.preview.kind).toBe("pdf");
      expect(snapshot.pdf?.relativePath).toBe("paper.pdf");

      const finishBuild = releaseBuild as (() => void) | null;
      if (finishBuild) {
        releaseBuild = null;
        finishBuild();
      }
