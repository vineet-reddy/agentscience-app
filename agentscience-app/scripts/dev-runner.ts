...
export const DEFAULT_AGENTSCIENCE_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(homedir(), ".agentscience"),
);
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("AGENTSCIENCE_PORT_OFFSET"),
  devInstance: optionalStringConfig("AGENTSCIENCE_DEV_INSTANCE"),
});
...
export function resolveOffset(config: {
  ...
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      throw new Error(`Invalid AGENTSCIENCE_PORT_OFFSET: ${config.portOffset}`);
    }
    return {
      offset: config.portOffset,
      source: `AGENTSCIENCE_PORT_OFFSET=${config.portOffset}`,
    };
  }
  ...
  if (/^\d+$/.test(seed)) {
    return { offset: Number(seed), source: `numeric AGENTSCIENCE_DEV_INSTANCE=${seed}` };
  }

  const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
  return { offset, source: `hashed AGENTSCIENCE_DEV_INSTANCE=${seed}` };
}
...
function resolveBaseDir(baseDir: string | undefined): Effect.Effect<string, never, Path.Path> {
  return Effect.gen(function* () {
    ...
    return yield* DEFAULT_AGENTSCIENCE_HOME;
  });
}
...
interface CreateDevRunnerEnvInput {
  ...
  readonly agentScienceHome: string | undefined;
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    const resolvedBaseDir = yield* resolveBaseDir(agentScienceHome);
    ...
    const output: NodeJS.ProcessEnv = {
      ...
      AGENTSCIENCE_HOME: resolvedBaseDir,
    };

    if (!isDesktopMode) {
      output.AGENTSCIENCE_PORT = String(serverPort);
      output.VITE_WS_URL = `ws://localhost:${serverPort}`;
    } else {
      delete output.AGENTSCIENCE_PORT;
      delete output.VITE_WS_URL;
      delete output.AGENTSCIENCE_AUTH_TOKEN;
      delete output.AGENTSCIENCE_MODE;
      delete output.AGENTSCIENCE_NO_BROWSER;
      delete output.AGENTSCIENCE_HOST;
    }

    if (!isDesktopMode && host !== undefined) {
      output.AGENTSCIENCE_HOST = host;
    }

    if (!isDesktopMode && authToken !== undefined) {
      output.AGENTSCIENCE_AUTH_TOKEN = authToken;
    } else if (!isDesktopMode) {
      delete output.AGENTSCIENCE_AUTH_TOKEN;
    }

    if (!isDesktopMode && noBrowser !== undefined) {
      output.AGENTSCIENCE_NO_BROWSER = noBrowser ? "1" : "0";
    } else if (!isDesktopMode) {
      delete output.AGENTSCIENCE_NO_BROWSER;
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.AGENTSCIENCE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.AGENTSCIENCE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.AGENTSCIENCE_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.AGENTSCIENCE_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.AGENTSCIENCE_MODE = "web";
      delete output.AGENTSCIENCE_DESKTOP_WS_URL;
    }
...
export function runDevRunnerWithInput(input: DevRunnerCliInput) {
  return Effect.gen(function* () {
    const { portOffset, devInstance } = yield* OffsetConfig.asEffect().pipe(
      Effect.mapError(
        (cause) =>
          new DevRunnerError({
            message:
              "Failed to read AGENTSCIENCE_PORT_OFFSET/AGENTSCIENCE_DEV_INSTANCE configuration.",
            cause,
          }),
      ),
    );
    ...
    const envOverrides = {
      noBrowser: readOptionalBooleanEnv("AGENTSCIENCE_NO_BROWSER"),
      autoBootstrapProjectFromCwd: readOptionalBooleanEnv(
        "AGENTSCIENCE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD",
      ),
      logWebSocketEvents: readOptionalBooleanEnv("AGENTSCIENCE_LOG_WS_EVENTS"),
    };
    ...
    const env = yield* createDevRunnerEnv({
      ...
      serverOffset,
      webOffset,
      agentScienceHome: input.agentScienceHome,
    ...
    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.AGENTSCIENCE_PORT)} webPort=${String(env.PORT)} baseDir=${String(env.AGENTSCIENCE_HOME)}`,
    );
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  authToken: Flag.string("auth-token").pipe(
    Flag.withDescription("Auth token (forwards to AGENTSCIENCE_AUTH_TOKEN)."),
    Flag.withAlias("token"),
    Flag.withFallbackConfig(optionalStringConfig("AGENTSCIENCE_AUTH_TOKEN")),
  ),
  noBrowser: Flag.boolean("no-browser").pipe(
    Flag.withDescription("Browser auto-open toggle (equivalent to AGENTSCIENCE_NO_BROWSER)."),
    Flag.withFallbackConfig(optionalBooleanConfig("AGENTSCIENCE_NO_BROWSER")),
  ),
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to AGENTSCIENCE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("AGENTSCIENCE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription(
      "WebSocket event logging toggle (equivalent to AGENTSCIENCE_LOG_WS_EVENTS).",
    ),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("AGENTSCIENCE_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to AGENTSCIENCE_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("AGENTSCIENCE_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to AGENTSCIENCE_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("AGENTSCIENCE_PORT")),
  ),
