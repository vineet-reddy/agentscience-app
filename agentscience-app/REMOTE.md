...
Use this when you want to open Agent Science from another device (phone, tablet, another laptop).

## CLI ↔ Env option map

The Agent Science CLI accepts the following configuration options, available either as CLI flags or environment variables:

| CLI flag                | Env var                     | Notes                                                                                |
| ----------------------- | --------------------------- | ------------------------------------------------------------------------------------ |
| `--mode <web\|desktop>` | `AGENTSCIENCE_MODE`         | Runtime mode.                                                                        |
| `--port <number>`       | `AGENTSCIENCE_PORT`         | HTTP/WebSocket port.                                                                 |
| `--host <address>`      | `AGENTSCIENCE_HOST`         | Bind interface/address.                                                              |
| `--base-dir <path>`     | `AGENTSCIENCE_HOME`         | Base directory.                                                                      |
| `--dev-url <url>`       | `VITE_DEV_SERVER_URL`       | Dev web URL redirect/proxy target.                                                   |
| `--no-browser`          | `AGENTSCIENCE_NO_BROWSER`   | Disable auto-open browser.                                                           |
| `--auth-token <token>`  | `AGENTSCIENCE_AUTH_TOKEN`   | WebSocket auth token. Use this for standard CLI and remote-server flows.             |
| `--bootstrap-fd <fd>`   | `AGENTSCIENCE_BOOTSTRAP_FD` | Read a one-shot bootstrap envelope from an inherited file descriptor during startup. |
