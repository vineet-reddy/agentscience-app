...
describe("extractPathFromShellOutput", () => {
  it("extracts the path between capture markers", () => {
    expect(
      extractPathFromShellOutput(
        "__AGENTSCIENCE_PATH_START__\n/opt/homebrew/bin:/usr/bin\n__AGENTSCIENCE_PATH_END__\n",
      ),
...
describe("readPathFromLoginShell", () => {
  it("uses a non-interactive login shell printenv PATH probe", () => {
    ...
    >(() => "__AGENTSCIENCE_ENV_PATH_START__\n/a:/b\n__AGENTSCIENCE_ENV_PATH_END__\n");
...
describe("readEnvironmentFromLoginShell", () => {
  it("extracts multiple environment variables from a login shell command", () => {
    ...
    >(() =>
      [
        ...
        "/a:/b",
        "__AGENTSCIENCE_ENV_PATH_END__",
        "__AGENTSCIENCE_ENV_SSH_AUTH_SOCK_START__",
        "/tmp/secretive.sock",
        "__AGENTSCIENCE_ENV_SSH_AUTH_SOCK_END__",
      ].join("\n"),
    );
  ...
  it("preserves surrounding whitespace in captured values", () => {
    ...
    >(() =>
      [
        "__AGENTSCIENCE_ENV_CUSTOM_VAR_START__",
        "  padded value  ",
        "__AGENTSCIENCE_ENV_CUSTOM_VAR_END__",
      ].join("\n"),
    );
